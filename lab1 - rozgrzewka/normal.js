...
const btnAddInputField = document.getElementById("#btnAdd");
const btnRemoveInputField = document.getElementById("#btnRemove");
let normalNumArr = [NaN, NaN, NaN, NaN];
let nonExistingIds = [4];

function liveCalculationNormal() {
  arrValid = removeInvalid(normalNumArr);
  let sumResult = arrValid.reduceRight(function (a, b) {
    return a + b;
  });
...
btnAddInputField.addEventListener("click", () => {
  if (nonExistingIds.length != 0) {
    ...
    input.setAttribute(
      "onkeyup",
      "passValue(" + passId + "); liveCalculationNormal()"
    );
...
btnRemoveInputField.addEventListener("click", () => {
